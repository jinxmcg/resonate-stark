...
qa = load_qa("prime"); tr = qa.get_idx_split()["train"].tolist(); EXTRA = json.load(open(a.extra)) if a.extra else {}; EXTRA2 = json.load(open(a.extra2)) if a.extra2 else {}
ex, held = [], []
...
for suf, QS in (("", {}), ("_para", EXTRA)):
    ...
    json.dump({int(qa[i][1]): top[j].tolist() for j, i in enumerate(held)}, open(f"data/oof/{a.tag}_f{k}{suf}.json", "w"))
