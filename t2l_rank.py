...
assert a.split in ("train", "val", "test", "test-0.1", "human_generated_eval")
assert not (a.queries and a.split not in ("train", "val")), "paraphrases are a development tool: train/val only"
...
human = a.split == "human_generated_eval"
qa = load_qa("prime", human_generated_eval=human); idx = qa.get_idx_split()[a.split].tolist() if not human else list(range(len(qa))); QS = json.load(open(a.queries)) if a.queries else {}
out = {}
