#!/bin/bash
# P2 committed read with the no-LLM pipeline. Dry run on val first, then test, test-0.1, human_generated_eval.
...
st() { echo "$(date +%H:%M:%S) $1" >> logs/committed_p2_status.log; }
run_split() {
  ...
  grep "wrote\|COMMITTED\|Error" logs/cr_predict_$S.log | sed "s/^/    /" >> logs/committed_p2_status.log
}
st "DRY RUN on val"
run_split val
grep -q "hit1': 0.41" logs/cr_predict_val.log || { st "DRY RUN MISMATCH — stopping before any test read"; exit 1; }
rm -f eval_results_val.csv
mkdir -p results_p2
for S in test test-0.1 human_generated_eval; do run_split $S; mv eval_results_$S.csv results_p2/ 2>/dev/null; done
st "COMMITTED_P2_DONE"
